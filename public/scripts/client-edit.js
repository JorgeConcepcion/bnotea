...
$('#behavior-add').on("keypress", function (event) {
	if (event.which === 13) {
		let index = 0;
		let element = $("#behavior-table .behavior-table-row:last-child input").attr("name");
		if (element != undefined) {
			index = Number(element.split("[")[2].split("]")[0]) + 1;
		}

		$("#behavior-table").append('<tr class="behavior-table-row table-row"></tr>');
...
$('#replacement-add').on("keypress", function (event) {
	if (event.which === 13) {
		let index = 0;
		let element = $("#replacement-table .replacement-table-row:last-child input").attr("name");
		if (element != undefined) {
			index = Number(element.split("[")[2].split("]")[0]) + 1;
		}
		console.log(index);
		$("#replacement-table").append('<tr class="replacement-table-row table-row"></tr>');
...
$('#approval-add').on("keypress", function (event) {
	if (event.which === 13) {
		let index = 0;
		let element = $("#approval-table .approval-table-row:last-child input").attr("name");
		if (element != undefined) {
			index = Number(element.split("[")[2].split("]")[0]) + 1;
		}
		console.log(index);
		$("#approval-table").append('<tr class="approval-table-row table-row"></tr>');
